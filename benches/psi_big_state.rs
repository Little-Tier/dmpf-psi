use dmpf::{
    big_state::BigStateDmpf, Dmpf, DmpfKey, DpfOutput, Node,
};
use rand::{thread_rng, RngCore, CryptoRng};
use std::fs;
use std::path::Path;
use std::time::Instant;
use std::marker::PhantomData; // 添加PhantomData导入
 
// PSI协议相关结构体
struct PsiProtocolSetup<Output: DpfOutput> {
    pub dmpf: BigStateDmpf,
    pub input_len: usize,
    pub threshold: usize,
    pub _phantom: PhantomData<Output>, // 添加PhantomData字段来使用Output参数
}
 
impl<Output: DpfOutput> PsiProtocolSetup<Output> {
    pub fn new(input_len: usize, threshold: usize) -> Self {
        // BigStateDmpf只需要batch_size参数，我们使用一个合理的默认值
        let dmpf = BigStateDmpf::new(8); // batch_size = 8
        Self {
            dmpf,
            input_len,
            threshold,
            _phantom: PhantomData, // 初始化PhantomData
        }
    }
}
...
) -> (Vec<u128>, Vec<u128>, Vec<u128>, Vec<u128>, dmpf::big_state::BigStateDmpfKey<Output>, dmpf::big_state::BigStateDmpfKey<Output>) {
...
fn online_phase_computation<Output: DpfOutput>(
    key0: &dmpf::big_state::BigStateDmpfKey<Output>,
    key1: &dmpf::big_state::BigStateDmpfKey<Output>,
...
const PSI_THRESHOLDS: [usize; 1] = [2050];
const PSI_SET_SIZES: [usize; 1] = [2048];
...
impl SingleTestResult {
    fn print_and_save(&self, input_len: usize, threshold: usize, set_size: usize) {
        ...
        // 保存到参数特定的文件
        let param_dir = format!("data/psi_big_state/input_{}_threshold_{}_set_{}", input_len, threshold, set_size);
